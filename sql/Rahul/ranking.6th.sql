...
SELECT orderid,prodname,amount,country,row_number() OVER
 (partition by country order by amount DESC) rnk FROM order_table;

SELECT orderid, prodname, amount, country, rnk FROM (
SELECT orderid,prodname,amount,country,row_number()
 OVER (partition by country order by amount DESC) rnk FROM order_table) abc 
WHERE rnk=1;

SELECT orderid,prodname,amount,country,rank() OVER ( order by amount DESC) rnk FROM order_table; 

SELECT orderid,prodname,amount,country,rank() OVER 
(partition by country order by amount DESC) rnk FROM order_table; 
